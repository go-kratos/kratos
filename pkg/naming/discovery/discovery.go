...
func (r *Resolver) Fetch(c context.Context) (ins map[string][]*naming.Instance, ok bool) {
	...
	r.d.mutex.RUnlock()
	if ok {
		var appIns naming.App
		appIns, ok = app.zoneIns.Load().(naming.App)
		if r.opt.Filter != nil {
			appIns.Instances = r.opt.Filter(appIns.Instances)
		}
		if r.opt.Scheduler != nil {
			appIns.Instances[r.opt.ClientZone] = r.opt.Scheduler(appIns)
		}
		if r.opt.Subset != nil && r.opt.SubsetSize != 0 {
			for zone, inss := range appIns.Instances {
				appIns.Instances[zone] = r.opt.Subset(inss, r.opt.SubsetSize)
			}
		}
		ins = appIns.Instances
		return
	}
	return
}
...
func (d *Discovery) polls(ctx context.Context, host string) (apps map[string]naming.App, err error) {
	var (
		lastTs  []int64
		appid   []string
		changed bool
	...
	res := new(struct {
		Code    int                   `json:"code"`
		Message string                `json:"message"`
		Data    map[string]naming.App `json:"data"`
	})
	params := url.Values{}
...
func (d *Discovery) broadcast(apps map[string]naming.App) {
	for id, v := range apps {
		var count int
