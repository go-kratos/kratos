...
func TestRegister(t *testing.T) {
	Convey("test register and cancel", t, func(cc C) {
		env.Hostname = "host_1"
		d := New(c)
		defer d.Close()
		ctx := context.TODO()
		...
		rs := d.Build(appdID)
		ch := rs.Watch()
		cc.So(ch, ShouldNotBeNil)
		<-ch
		ins, ok := rs.Fetch(ctx)
		cc.So(ok, ShouldBeTrue)
		cc.So(err, ShouldBeNil)
		var count int
		for _, data := range ins {
			count += len(data)
		}
		cc.So(count, ShouldEqual, 1)
		c.Host = "host_2"
		env.Hostname = "host_2"
		...
		cc.So(count, ShouldEqual, 1)
		Convey("test discovery set", func(cc C) {
			c.Host = "host_1"
			...
			cc.So(ins["sh001"][0].Metadata["weight"], ShouldResemble, "111")
		})
	})
}

func TestMultiZone(t *testing.T) {
	Convey("test multi zone", t, func(cc C) {
		env.Hostname = "host_1"
		d := New(c)
		defer d.Close()
		ctx := context.TODO()
		...
		cc.So(count, ShouldEqual, 2)
		cc.So(zoneCount, ShouldEqual, 2)
	})
}

func TestDiscoveryFailOver(t *testing.T) {
	Convey("test failover", t, func(cc C) {
		var conf = &Config{
		...
		for _, a := range []string{":8080"} {
			go func(addr string) {
				...
				e.GET("/discovery/polls", func(ctx *blademaster.Context) {
					...
					if ts == "0" {
						ctx.JSON(map[string]naming.App{
							appdID:            {LastTs: time.Now().UnixNano(), Instances: map[string][]*naming.Instance{"zone": {in}}},
							"infra.discovery": {LastTs: time.Now().UnixNano(), Instances: map[string][]*naming.Instance{conf.Zone: {indis}}},
						}, nil)
					} else {
		...
		cc.So(count, ShouldEqual, 1)
		cc.So(zoneCount, ShouldEqual, 1)
	})
}

func TestWatchContinuosly(t *testing.T) {
	Convey("test TestWatchContinuosly ", t, func(cc C) {
		env.Hostname = "host_1"
		d := New(c)
		defer d.Close()
		in1 := *in
		...
		cc.So(count, ShouldBeGreaterThanOrEqualTo, 1)
...
func TestSameBuilder(t *testing.T) {
	Convey("test multi watch", t, func(cc C) {
		env.Hostname = "host_1"
		d := New(c)
		defer d.Close()
		ctx := context.TODO()
