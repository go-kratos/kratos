...
func ScheduleNode(clientZone string) BuildOpt {
	return &funcOpt{f: func(opt *BuildOptions) {
		...
		opt.Scheduler = func(app App) (instances []*Instance) {
			type Zone struct {
				inss   []*Instance
				weight int64
				name   string
...
func ScheduleScore(clientZone string) BuildOpt {
	return &funcOpt{f: func(opt *BuildOptions) {
		opt.ClientZone = clientZone
		opt.Scheduler = func(app App) (instances []*Instance) {
			type Zone struct {
			...
			if app.Scheduler != nil {
				// 拿到客户端所在zone的调度策略
				if strategy, ok := app.Scheduler.Clients[clientZone]; ok {
					// 拿到所有zone的权重、zone的所有节点、zone的名字
					for name, zone := range strategy.Zones {
						inss := app.Instances[name]
						if len(inss) > 0 {
							zones = append(zones, &Zone{inss: inss, weight: zone.Weight, name: name})
						}
					}
					if len(zones) > 0 && opt.SubsetSize > 0 && opt.Subset != nil {
						sort.Slice(zones, func(i, j int) bool {
							return len(zones[i].inss) < len(zones[j].inss)
						})
						// 根据subsetSize得到每个zone应给被分配的节点数
						selPerZone := opt.SubsetSize/len(zones) + 1
						remain := opt.SubsetSize
						for i, zone := range zones {
							// 根据selPerZone进行节点筛选
							if len(zone.inss) > selPerZone {
								zone.inss = opt.Subset(zone.inss, selPerZone)
								remain -= len(zone.inss)
							} else if i != (len(zones) - 1) {
								remain -= len(zone.inss)
								selPerZone = remain / (len(zones) - i - 1)
							}
						}
					}
				}
			...
			for _, zone := range zones {
				for _, ins := range zone.inss {
					...
					//根据zone的权重和节点本身的权重得到最终的权重值
					weight *= int64(zone.weight) * 1000 / int64(len(zone.inss))
