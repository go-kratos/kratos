...
func SwaggerHandler(url string) bm.HandlerFunc {
	u := ginSwagger.URL(url)
	return swaggerHandler(swaggerFiles.Handler, u)
}

// SwaggerHandler wraps `http.Handler` into `bm.HandlerFunc`.
func swaggerHandler(h *webdav.Handler, confs ...func(c *ginSwagger.Config)) bm.HandlerFunc {
