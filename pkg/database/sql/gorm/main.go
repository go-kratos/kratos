...
type slowLogModeValue int

var defaultSlowTime int64 = 1000
...
const (
	defaultSlowLogMode slowLogModeValue = iota
	noSlowLogMode
	detailedSlowLogMode
)
...
// SlowLogMode set log mode, `true` for detailed logs, `false` for no log, default, will only print error logs
// slowTime（毫秒）
func (s *DB) SlowLogMode(enable bool, slowTime ...int64) *DB {
	if enable {
		s.slowLogMode = detailedSlowLogMode
		if len(slowTime) == 1 && slowTime[0] >= 0 {
			s.slowTime = slowTime[0]
		} else {
			s.slowTime = defaultSlowTime
		}
	} else {
		s.slowLogMode = noSlowLogMode
	}
	return s
}
...
func (s *DB) NewScopeWithCtx(ctx context.Context, value interface{}) *Scope {
	dbClone := s.clone()
	dbClone.Value = value
	scope := &Scope{db: dbClone, Value: value, C: ctx}
...
func (s *DB) First(ctx context.Context, out interface{}, where ...interface{}) *DB {
	...
	if ctx == nil {
		...
		newScope = s.NewScopeWithCtx(ctx, out)
	}
...
func (s *DB) Take(ctx context.Context, out interface{}, where ...interface{}) *DB {
	var newScope *Scope
	if ctx == nil {
		newScope = s.NewScope(out)
	} else {
...
func (s *DB) Preloads(ctx context.Context, out interface{}) *DB {
	var newScope *Scope
	if ctx == nil {
		newScope = s.NewScope(out)
	} else {
	...
	return newScope.InstanceSet("gorm:only_preload", 1).callCallbacks(s.parent.callbacks.queries).db
}
...
func (s *DB) Scan(ctx context.Context, dest interface{}) *DB {
	var newScope *Scope
	if ctx == nil {
		newScope = s.NewScope(s.Value)
	} else {
		newScope = s.NewScopeWithCtx(ctx, s.Value)
	}
	return newScope.Set("gorm:query_destination", dest).callCallbacks(s.parent.callbacks.queries).db
}
...
func (s *DB) Rows(ctx context.Context) (*sql.Rows, error) {
	...
	return newScope.rows()
}

// ScanRows scan `*sql.Rows` to give struct
func (s *DB) ScanRows(ctx context.Context, rows *sql.Rows, result interface{}) error {
	var newScope *Scope
	if ctx == nil {
		newScope = s.NewScope(result)
	} else {
		newScope = s.NewScopeWithCtx(ctx, result)
	}
	...
	if clone.AddError(err) == nil {
		newScope.scan(rows, columns, newScope.Fields())
	}
...
func (s *DB) Pluck(ctx context.Context, column string, value interface{}) *DB {
	...
	return newScope.pluck(column, value).db
}
...
func (s *DB) Count(ctx context.Context, value interface{}) *DB {
	...
	return newScope.count(value).db
}
...
func (s *DB) Related(ctx context.Context, value interface{}, foreignKeys ...string) *DB {
	...
	return newScope.related(value, foreignKeys...).db
}
...
func (s *DB) FirstOrInit(ctx context.Context, out interface{}, where ...interface{}) *DB {
	...
	if result := c.First(ctx, out, where...); result.Error != nil {
		...
		if ctx == nil {
			...
			c.NewScopeWithCtx(ctx, out).inlineCondition(where...).initialize()
		}
	} else {
		if ctx == nil {
			c.NewScope(out).updatedAttrsWithValues(c.search.assignAttrs)
		} else {
			c.NewScopeWithCtx(ctx, out).updatedAttrsWithValues(c.search.assignAttrs)
		}
	}
	return c
}
...
func (s *DB) FirstOrCreate(ctx context.Context, out interface{}, where ...interface{}) *DB {
	c := s.clone()
	if result := s.First(ctx, out, where...); result.Error != nil {
		if !result.RecordNotFound() {
			return result
		}
		if ctx == nil {
			...
			return c.NewScopeWithCtx(ctx, out).inlineCondition(where...).initialize().callCallbacks(c.parent.callbacks.creates).db
		}
		...
		if ctx == nil {
			...
			return c.NewScopeWithCtx(ctx, out).InstanceSet("gorm:update_interface", c.search.assignAttrs).callCallbacks(c.parent.callbacks.updates).db
		}
	}
	return c
}
...
func (s *DB) Updates(ctx context.Context, values interface{}, ignoreProtectedAttrs ...bool) *DB {
	if ctx == nil {
		...
		return s.NewScopeWithCtx(ctx, s.Value).
			Set("gorm:ignore_protected_attrs", len(ignoreProtectedAttrs) > 0).
			InstanceSet("gorm:update_interface", values).
			callCallbacks(s.parent.callbacks.updates).db
	}
...
func (s *DB) Save(ctx context.Context, value interface{}) *DB {
	...
	if ctx == nil {
		...
		scope = s.NewScopeWithCtx(ctx, value)
	}
	if !scope.PrimaryKeyZero() {
		newDB := scope.callCallbacks(s.parent.callbacks.updates).db
		if newDB.Error == nil && newDB.RowsAffected == 0 {
			return s.New().Table(scope.TableName()).FirstOrCreate(ctx, value)
		}
		return newDB
	}
...
func (s *DB) Create(ctx context.Context, value interface{}) *DB {
	if ctx == nil {
		return s.NewScope(value).callCallbacks(s.parent.callbacks.creates).db
	} else {
		return s.NewScopeWithCtx(ctx, value).callCallbacks(s.parent.callbacks.creates).db
	}
}
...
func (s *DB) Delete(ctx context.Context, value interface{}, where ...interface{}) *DB {
	if ctx == nil {
		...
		return s.NewScopeWithCtx(ctx, value).inlineCondition(where...).callCallbacks(s.parent.callbacks.deletes).db
	}
}
...
func (s *DB) Exec(ctx context.Context, sql string, values ...interface{}) *DB {
	...
	if ctx == nil {
		...
		scope = s.NewScopeWithCtx(ctx, nil)
	}
...
func (s *DB) NewRecord(ctx context.Context, value interface{}) bool {
	if ctx == nil {
		...
		return s.NewScopeWithCtx(ctx, value).PrimaryKeyZero()
	}
}
...
func (s *DB) Association(ctx context.Context, column string) *Association {
	...
	if ctx == nil {
		...
		scope = s.Set("gorm:association:source", s.Value).NewScopeWithCtx(ctx, s.Value)
	}
...
func (s *DB) slog(sql string, t time.Time, vars ...interface{}) {
	...
	if s.logMode == detailedLogMode {
		s.print("sql", fileWithLineNum(), _time, sql, vars, s.RowsAffected)
	}
	if s.slowLogMode == detailedSlowLogMode && _time.Milliseconds() > s.slowTime {
		s.print("slow_sql", fileWithLineNum(), _time, sql, vars, s.RowsAffected)
	}
}
