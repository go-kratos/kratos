...
func (association *Association) Replace(values ...interface{}) *Association {
	...
	if relationship.Kind == "belongs_to" {
		...
		if len(values) == 0 {
			...
			association.setErr(newDB.Model(scope.Value).UpdateColumn(association.scope.C, foreignKeyMap).Error)
		}
	} else {
		...
		if relationship.Kind == "many_to_many" {
			...
			if sourcePrimaryKeys := scope.getColumnAsArray(sourceForeignFieldNames, scope.Value); len(sourcePrimaryKeys) > 0 {
				...
				association.setErr(relationship.JoinTableHandler.Delete(association.scope.C, relationship.JoinTableHandler, newDB))
			}
			...
			association.setErr(newDB.Model(fieldValue).UpdateColumn(association.scope.C, foreignKeyMap).Error)
		}
	}
	return association
}
...
func (association *Association) Delete(values ...interface{}) *Association {
	...
	if relationship.Kind == "many_to_many" {
		...
		if relationship.Kind == "belongs_to" {
			...
			if results := newDB.Model(modelValue).UpdateColumn(association.scope.C, foreignKeyMap); results.Error == nil {
...
func (association *Association) Count() int {
	...
	if err := query.Model(fieldValue).Count(association.scope.C, &count).Error; err != nil {
	...
	saveAssociation := func(reflectValue reflect.Value) {
		...
		if relationship.Kind == "many_to_many" {
			if scope.New(reflectValue.Interface()).PrimaryKeyZero() {
				association.setErr(scope.NewDB().Save(association.scope.C, reflectValue.Interface()).Error)
			}
		}
		...
		if relationship.Kind == "many_to_many" {
			association.setErr(relationship.JoinTableHandler.Add(association.scope.C, relationship.JoinTableHandler, scope.NewDB(), scope.Value, reflectValue.Interface()))
		} else {
			association.setErr(scope.NewDB().Select(field.Name).Save(association.scope.C, scope.Value).Error)
