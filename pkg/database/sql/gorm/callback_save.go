	...
	for _, field := range scope.Fields() {
		...
		if relationship != nil && relationship.Kind == "belongs_to" {
			...
			if newScope.PrimaryKeyZero() {
				if autoCreate {
					scope.Err(scope.NewDB().Save(scope.C, fieldValue).Error)
				}
			} else if autoUpdate {
				scope.Err(scope.NewDB().Save(scope.C, fieldValue).Error)
			}
		...
		if relationship != nil && (relationship.Kind == "has_one" || relationship.Kind == "has_many" || relationship.Kind == "many_to_many") {
			...
			case reflect.Slice:
				for i := 0; i < value.Len(); i++ {
					...
					if newScope.PrimaryKeyZero() {
						if autoCreate {
							scope.Err(newDB.Save(scope.C, elem).Error)
						}
					} else if autoUpdate {
						scope.Err(newDB.Save(scope.C, elem).Error)
					}
					...
					if !scope.New(newScope.Value).PrimaryKeyZero() && saveReference {
						if joinTableHandler := relationship.JoinTableHandler; joinTableHandler != nil {
							scope.Err(joinTableHandler.Add(scope.C, joinTableHandler, newDB, scope.Value, newScope.Value))
						}
					}
				}
			default:
				...
				if newScope.PrimaryKeyZero() {
					if autoCreate {
						scope.Err(scope.NewDB().Save(scope.C, elem).Error)
					}
				} else if autoUpdate {
					scope.Err(scope.NewDB().Save(scope.C, elem).Error)
				}
