		...
		if fromField != nil {
			if relationship := fromField.Relationship; relationship != nil {
				if relationship.Kind == "many_to_many" {
					...
					scope.Err(joinTableHandler.JoinWith(joinTableHandler, tx, scope.Value).Find(scope.C, value).Error)
					...
					scope.Err(tx.Find(scope.C, value).Error)
				...
				scope.Err(tx.Where(sql, fromField.Field.Interface()).Find(scope.C, value).Error)
			}
			return scope
		} else if toField != nil {
			sql := fmt.Sprintf("%v = ?", scope.Quote(toField.DBName))
			scope.Err(tx.Where(sql, scope.PrimaryKeyValue()).Find(scope.C, value).Error)
			return scope
		}
	}
...
func (scope *Scope) createJoinTable(field *StructField) {
	if relationship := field.Relationship; relationship != nil && relationship.JoinTableHandler != nil {
		...
		if !scope.Dialect().HasTable(joinTable) {
			...
			scope.Err(scope.NewDB().Exec(scope.C, fmt.Sprintf("CREATE TABLE %v (%v, PRIMARY KEY (%v))%s", scope.Quote(joinTable), strings.Join(sqlTypes, ","), strings.Join(primaryKeys, ","), scope.getTableOptions())).Error)
		}
