...
func TestJoinTable(t *testing.T) {
	DB.Exec(nil, "drop table person_addresses;")
	...
	if DB.Unscoped().Find(nil, &[]PersonAddress{}, "person_id = ?", person.Id).RowsAffected != 2 {
...
func TestEmbeddedMany2ManyRelationship(t *testing.T) {
	...
	if err := DB.Save(nil, person).Error; err != nil {
