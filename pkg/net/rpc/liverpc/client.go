...
// Transport 可以理解为一个通道。Client通过Transport进行通信。
type Transport interface {
	Call(ctx context.Context, version int, serviceMethod string, in proto.Message, out proto.Message, opts ...CallOption) (err error)
	CallRaw(ctx context.Context, version int, serviceMethod string, in *Args, opts ...CallOption) (out *Reply, err error)
	Close()
}
...
func NewClient(c *ClientConfig) *Client {
	cli := &Client{newDefaultTransport(c)}
	return cli
}

func newDefaultTransport(c *ClientConfig) *defaultTransport {
	if c.Timeout <= 0 {
...
func (c *defaultTransport) disproc(event <-chan struct{}) {
	for {
		_, ok := <-event
		if !ok {
			return
...
func (c *defaultTransport) disFetch() {
...
func (c *defaultTransport) pickConn(ctx context.Context, addrs []string, dialTimeout time.Duration) (*ClientConn, error) {
	var (
		lastErr error
	)
	if len(addrs) == 0 {
		...
		for i := 0; i < _dialRetries; i++ {
			...
			cc, err := Dial(ctx, "tcp", addr, time.Duration(c.conf.Timeout), dialTimeout, c.conf.AppID)
			if err != nil {
...
func (c *defaultTransport) fetchAddrs(ctx context.Context, request interface{}) (addrs []string, err error) {
	var (
		args        *Args
	...
	if env.Color != "" && env.Color != metaColor && env.Color != sourceGroup {
...
func (c *defaultTransport) Call(ctx context.Context, version int, serviceMethod string, in proto.Message, out proto.Message, opts ...CallOption) (err error) {
	var (
		cc    *ClientConn
		addrs []string
	)
...
// httpURL: /room/v1/Room/room_init
// httpURL: /{service}/{version}/{controller}/{method}
func (c *defaultTransport) CallRaw(ctx context.Context, version int, serviceMethod string, in *Args, opts ...CallOption) (out *Reply, err error) {
	var (
		cc    *ClientConn
		addrs []string
	)
