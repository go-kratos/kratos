...
func startTestServer() {
	...
	engine.GET("/mirrortest", func(ctx *Context) {
		ctx.JSON(metadata.String(ctx, metadata.Mirror), nil)
	})
	engine.Start()
}
...
type Archive struct {
	...
	HeaderXTestMark    string   `header:"x-test-mark"`
	HeaderXTestInt     int64    `header:"x-test-int"`
	HeaderXTestDefault int64    `header:"x-test-default" default:"300"`
...
func setupHandler(engine *Engine) {
	...
	engine.GET("/meta/:name", func(ctx *Context) {
...
func TestJSONPEscape(t *testing.T) {
	once.Do(startServer)
	...
	testCase := []struct {
		...
		{
			`<body onload="alert('h4x0rd');"/><!--`,
			`\x3Cbody onload=\"alert(\'h4x0rd\');\"/\x3E\x3C!--`,
		},
	}

	for _, tc := range testCase {
		q := url.Values{}
		q.Set("cross_domain", "true")
		q.Set("jsonp", "jsonp")
		q.Set("callback", tc.callback)

		req, err := http.NewRequest("GET", uri(SockAddr, "/json")+"?"+q.Encode(), nil)
		assert.NoError(t, err)
		...
		assert.Equal(t, tc.expected, string(matched[1]))
	}
}
...
func TestBind(t *testing.T) {
	...
	assert.NoError(t, err)
	req.Header.Set("X-TEST-MARK", "dangerous")
	req.Header.Set("X-TEST-iNT", "100")
	...
	assert.Equal(t, int64(100), p.Data.HeaderXTestInt)
	assert.Equal(t, "dangerous", p.Data.HeaderXTestMark)
	assert.Equal(t, int64(300), p.Data.HeaderXTestDefault)
	assert.NotNil(t, p.Data.RequestURL)
	assert.Equal(t, "/bind", p.Data.RequestURL.Path)
}
...
func TestBindWith(t *testing.T) {
	...
	assert.Equal(t, int64(100), p.Data.HeaderXTestInt)
	assert.Equal(t, "aaaa", p.Data.HeaderXTestMark)
	assert.NotNil(t, p.Data.RequestURL)
	assert.Equal(t, "/intest", p.Data.RequestURL.Path)
	assert.Equal(t, int64(0), p.Data.HeaderXTestDefault)
}
...
func TestNewServerMiddleware(t *testing.T) {
	e := DefaultServer(nil)
	// should contain 7 default handlers
	assert.Len(t, e.RouterGroup.Handlers, 7)
}
...
func TestServerWithMirror(t *testing.T) {
	...
	reqest, _ = http.NewRequest("GET", "http://localhost:18090/mirrortest", nil)
	reqest.Header.Add("x1-bilispy-mirror", "")
	resp, err = client.Do(reqest)
	...
	err = json.Unmarshal(bs, &response)
	assert.NoError(t, err)
	assert.Equal(t, "xxxxxxxxxx", response.Data)
	resp.Body.Close()
...
// TestEnginePathPrams is
func TestEnginePathPrams(t *testing.T) {
	once.Do(startServer)

	client := &http.Client{}
	resp, err := client.Get(uri(SockAddr, "/meta/midone"))
	assert.NoError(t, err)
	bs, _ := ioutil.ReadAll(resp.Body)
	out := struct {
		Code int64
	...
	assert.NoError(t, json.Unmarshal([]byte(bs), &out))
	assert.Equal(t, out.Data, map[string]string{
		"ReqMethod": "GET",
		"RoutePath": "/meta/:name",
		"method":    "GET",
		"ReqPath":   "/meta/midone",
...
func TestPerf(t *testing.T) {
	...
	for _, p := range testPath {
		resp, err := http.Get(uri(SockAddr, p))
		assert.NoError(t, err)
		assert.NotNil(t, resp)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()
