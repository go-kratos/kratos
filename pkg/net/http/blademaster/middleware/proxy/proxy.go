...
func (e *endpoint) ServeHTTP(ctx *bm.Context) {
	...
	logArgs := []log.D{
		...
		log.KVString("ip", ip),
		log.KVString("path", req.URL.Path),
		log.KVString("params", req.Form.Encode()),
	}
	if !e.condition(ctx) {
		logArgs = append(logArgs, log.KVString("proxied", "false"))
		log.Infov(ctx, logArgs...)
		return
	}
	logArgs = append(logArgs, log.KVString("proxied", "true"))
	log.Infov(ctx, logArgs...)
	...
	defer func() {
		...
		switch r.(type) {
		case error:
			if err := r.(error); err == http.ErrAbortHandler {
				logArgs = append(logArgs, log.KV("proxy-error", err))
				log.Errorv(ctx, logArgs...)
				return
			}
		default:
			panic(r)
...
// NewYlfProxy is
func NewYlfProxy(dst string) bm.HandlerFunc {
	ep := newep(dst, func(*bm.Context) bool {
		return env.Zone != "sh001"
	})
	return ep.ServeHTTP
}
