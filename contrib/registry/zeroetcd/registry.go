...
func (r *Registry) Register(ctx context.Context, service *registry.ServiceInstance) error {
	// TODO go-zero
	key := service.Name
	value, err := ParseEndpoint(service.Endpoints, "grpc", false)
	if err != nil {
		return err
	}
	if r.lease != nil {
...
func (r *Registry) Deregister(ctx context.Context, service *registry.ServiceInstance) error {
	...
	//TODO make go-zero key
	key := service.Name
	fullKey := makeEtcdKey(key, int64(r.leaseID))
...
func (r *Registry) GetService(ctx context.Context, name string) ([]*registry.ServiceInstance, error) {
	...
	for _, kv := range resp.Kvs {
		...
		si := &registry.ServiceInstance{
			Name:      kvName,
			Endpoints: []string{fmt.Sprintf("grpc://%s",string(kv.Value))},
		}
...
func (r *Registry) registerWithKV(ctx context.Context, key string, value string) (clientv3.LeaseID, error) {
	...
	//TODO make go-zero key
	fullKey := makeEtcdKey(key, int64(r.leaseID))
	_, err = r.client.Put(ctx, fullKey, value, clientv3.WithLease(r.leaseID))
	if err != nil {
		return 0, err
	}
